...
The default limits are four concurrent requests, 500 pages per origin, eight
redirects per URL, a 10-second response timeout, 2 MB per HTML response, and
500 links/images retained per page. View every option with:
...
created with owner-only permissions, but they should still be treated as
operational artifacts and reviewed before sharing.
...
and `nofollow` polarity while continuing to compare every other robots
directive. Omit the flag for a production comparison so an accidental
production `noindex` remains a hard failure.
...
`noindex`, temporary redirect, lost image/alt text, old-origin media hotlink, or
crawl failure. Warnings identify review items such as changed headings,
