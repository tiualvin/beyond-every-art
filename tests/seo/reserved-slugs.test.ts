...
function metadataRootRouteSegments(directory: string): string[] {
  const metadataRoutes: Record<string, string> = {
    // The App Router serves the favicon straight from the file, so the key and
    // the segment are the same string here — unlike the two below, where the
    // route is generated by a module and named nothing like it.
    'icon.png': 'icon.png',
