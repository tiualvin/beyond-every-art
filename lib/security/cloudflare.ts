...
// The `Caddyfile` sets no `header_up` and no `trusted_proxies`, so that default
// append is what actually runs; a future directive that rewrites the header
// instead of appending to it would move the peer, and this module with it.
 ...
 *
 * **Verified 15 Sep 2026** against both published lists and against
 * `api.cloudflare.com/client/v4/ips`, which returned etag
 * `38f79d050aa027e3be3865e495dcc9bc` for exactly these 15 IPv4 and 7 IPv6
 * entries. That check is the one the drafted proposal was held on; re-run it
 * before the pass-two firewall edit, since both take the same list.
 *
 ...
 * malformed line fails on the way in rather than silently matching nothing, and
 * `tests/docs/drift.test.ts` holds `EDGE_PROTECTION.md` to the same list, so the
 * firewall rules and the limiter cannot drift apart without a test saying so.
 */
